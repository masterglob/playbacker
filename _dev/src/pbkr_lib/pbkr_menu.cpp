...
void PlayMenuItem::onExit(void)
{
    ...
    {
        globalMenuInstance().setMenu(&mainMenuItem);
    }
}
...
DeleteInternalProjectMenuItem::onExit(void)
{
    ...
    case S_CANCELED:
    {
        ...
        if (m_deleter)
            m_deleter->cancel();
        globalMenuInstance().setMenu(m_parent);
    }
    break;
    default:
        m_step = S_CANCELED;
...
SelectProjectShowMenuItem::onEnter(const bool longPressed)
{
    if (m_projectTitle.length() > 0)
    {
        ...
        {
            fileManager.loadProject(p);
            globalMenuInstance().setMenu(&playMenuItem);
...
void MenuItem::onEnter(const bool longPressed)
{
    if (m_iter != m_subMenus.end())
    {
        ...
        globalMenuInstance().setMenu(menuItem);
    }
}
...
MainMenu& globalMenuInstance(){return MainMenu::instance();}

void openCopyFromUSBMenu(){ globalMenuInstance().setMenu(&copyFromUSBMenuItem);}
void openNetMenuFromUSBMenu(){ globalMenuInstance().setMenu(&netMenuItem);}
void openDeleteProjectMenu(){ globalMenuInstance().setMenu(&deleteInternalProjectMenuItem);}
